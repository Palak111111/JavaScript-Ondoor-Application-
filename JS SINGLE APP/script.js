...
function headerComponent() {
    ...
    headerDiv.setAttribute("class", "container-fluid");
    headerDiv.setAttribute("style", "height:70px; position: sticky; top: 0; z-index: 100; overflow: hidden; background-color:#2a2a2a");

    var innerHeadDiv = document.createElement("div");
    innerHeadDiv.setAttribute("class", "row");

    var logoDiv = document.createElement('div');
    logoDiv.setAttribute("class", "col-md-2");

    var logoImgDiv = document.createElement('img');
    logoImgDiv.setAttribute("src", "logo1.jpg");
    logoImgDiv.setAttribute("style", "height:69px; width:60%");

    logoDiv.appendChild(logoImgDiv);
    innerHeadDiv.appendChild(logoDiv);

    // Add Heading "On-door Shopping"
    var headingDiv = document.createElement("div");
    headingDiv.setAttribute("class", "col-md-4 d-flex justify-content-center align-items-center");
    headingDiv.setAttribute("style", "height:70px;");
    
    var heading = document.createElement("h1");
    heading.innerText = "On-door Shopping";
    heading.setAttribute("style", "color: white; margin: 0; font-size: 24px;");
    
    headingDiv.appendChild(heading);
    innerHeadDiv.appendChild(headingDiv);

    var searchBarDiv = document.createElement("div");
    searchBarDiv.setAttribute("class", "col-md-4 d-flex justify-content-center align-items-center");
    searchBarDiv.setAttribute("style", "height:70px;");

    var searchInput = document.createElement('input');
    searchInput.setAttribute("id", "search");
    searchInput.setAttribute("type", "text");
    searchInput.setAttribute("placeholder", "Search Your Product");
    searchInput.setAttribute("style", "width:350px; height:35px; font-size:18px; border-radius:5px;text-align:center");

    searchBarDiv.appendChild(searchInput);

    var searchButton = document.createElement('button');
    searchButton.textContent = 'Search';
    searchButton.setAttribute("style", "width: 80px; height: 35px; font-size: 16px; float:right; right:10px;cursor:pointer; border-radius:5px;");

    searchBarDiv.appendChild(searchButton);

    var searchIcon = document.createElement('i');
    searchIcon.setAttribute('class', 'fa fa-search');
    searchIcon.setAttribute('style', 'font-size:15px;float:left; margin-top: 5px;');

    searchButton.appendChild(searchIcon);

    innerHeadDiv.appendChild(searchBarDiv);

    var option = document.createElement('div');
    option.setAttribute("style", "height:70px");
    option.setAttribute("class", "col-md-2 d-flex justify-content-around align-items-center");
    option.setAttribute('id', 'option-div');

    if (sessionStorage.getItem("isLoggedIn")) {
        option.innerHTML = "";
        var viewCart = document.createElement("label");
        viewCart.innerText = "View cart";
        viewCart.setAttribute('style', 'text-align: center; width: 80px; height: 30px; border-radius: 8px; font-size: 16px; cursor: pointer; background-color: #F2AA4CFF; border: none; color: black;');
        ...
        option.appendChild(viewCart);

        var signOutLabel = document.createElement("label");
        signOutLabel.innerText = "Sign out";
        signOutLabel.setAttribute('style', 'text-align: center; width: 80px; height: 30px; border-radius: 8px; font-size: 16px; cursor: pointer; background-color: #F2AA4CFF; border: none; color: black;');
        signOutLabel.addEventListener('click', () => {
            performSignOut();
        });
        option.appendChild(signOutLabel);

    }
    else {
        option.innerHTML = "";
        var signinLink = document.createElement('button');
        signinLink.setAttribute('style', 'text-align: center; width: 80px; height: 30px; border-radius: 8px; font-size: 16px; cursor: pointer; background-color: #F2AA4CFF; border: none; color: black;');
        signinLink.innerText = 'Sign-In';

        option.appendChild(signinLink);
        ...
        var signupLink = document.createElement('button');
        signupLink.setAttribute('style', 'text-align: center; width: 80px; height: 30px; border-radius: 8px; font-size: 16px; cursor: pointer; background-color: #F2AA4CFF; border: none; color: black;');
        signupLink.innerText = 'Sign-Up';

        signupLink.addEventListener("click", () => {
            var mainDiv = document.querySelector("#main");
            // mainDiv.removeChild(document.querySelector("#card-container"));//to remove all product from card component
            if (!signup) {
                signup = true;
        ...
        option.appendChild(signupLink);

    }

    innerHeadDiv.appendChild(option);

    headerDiv.appendChild(innerHeadDiv);
...
function signupComponent(mainDiv) {
    ...
    if (signin) {
        document.querySelector("#SignInformTagID").remove();
        signin = false;
    }
    var formTag = document.createElement('form');
    formTag.setAttribute('style', 'text-align: center; height: 300px; width: 300px; background-color: #E1EBEE; border-image-slice: 1; margin: 50px auto; padding-top: 10px; position: relative; box-shadow: 5px 5px 10px rgba(0,0,0,0.5);');
    formTag.id = 'signupform';
    var heading = document.createElement('h2');
    heading.setAttribute('style', 'margin-top: 30px; cursor: pointer;');
    heading.textContent = 'Sign-up';

    var underline = document.createElement('span');
    underline.setAttribute('style', 'display: block; height: 2px; margin-left: 100px; width: 0; background: #000; transition: width 0.3s ease-in-out;');
    ...
    formTag.appendChild(lineBreak1);

    var emailInput = document.createElement('input');
    emailInput.setAttribute('type', 'email');
    emailInput.setAttribute('placeholder', 'Enter your email');
    emailInput.setAttribute('id', 'email');
    emailInput.setAttribute('style', 'width: 180px; height: 30px; border-radius: 6px; border: 2px solid #004792; font-size: 18px;');
    formTag.appendChild(emailInput);

    var lineBreak2 = document.createElement('br');
    formTag.appendChild(lineBreak2);

    var passwordInput = document.createElement('input');
    passwordInput.setAttribute('type', 'password');
    passwordInput.setAttribute('id', 'password');
    passwordInput.setAttribute('placeholder', 'Enter your password');
    passwordInput.setAttribute('style', 'width: 180px; margin-top: 20px; height: 30px; border-radius: 6px; border: 2px solid #004792; font-size: 18px;');
    formTag.appendChild(passwordInput);
    ...
    confirmPasswordInput.setAttribute('style', 'width: 180px; margin-top: 20px; height: 30px; border-radius: 6px; border: 2px solid #004792; font-size: 18px;');
    formTag.appendChild(confirmPasswordInput);
    ...
    function checkPasswordMatch() {
        ...
        if (password !== confirmPassword) {
            if (!errorDiv) {
                ...
                formTag.appendChild(errorDiv);
            }
            errorDiv.textContent = 'Confirm-Password do not match';
        } else {
            if (errorDiv) {
    ...
    formTag.appendChild(lineBreak3);
    ...
    submitButton.style.height = "28px";
    submitButton.style.cursor = "pointer";
    submitButton.style.backgroundColor = "#F2AA4CFF";
    ...
    submitButton.addEventListener("click", () => {
        ...
        var password = passwordInput.value;
        var confirmPassword = confirmPasswordInput.value;
        var user = { email, password, confirmPassword };
        var userList = JSON.parse(localStorage.getItem("user-list"));
        var status = userList.find((user) => { return user.email == email && user.password == password && user.confirmPassword == confirmPassword });
        if (status)
...
function SignInComponent(mainDiv) {
    ...
    var formTag = document.createElement('form');
    formTag.setAttribute('style', 'text-align: center; height: 290px; width: 300px; background-color: #E1EBEE; border-image-slice: 1; margin: 50px auto; padding-top: 10px; position: relative; box-shadow: 5px 5px 10px rgba(0,0,0,0.5);');
    formTag.setAttribute('id', 'SignInformTagID')
    ...
    var underline = document.createElement('span');
    underline.setAttribute('style', 'display: block; height: 2px; margin-left: 140px; width: 0; background: #000; transition: width 0.3s ease-in-out;');
    ...
    submitButton.addEventListener("click", () => {
        ...
        if (user) {
            ...
            // alert('')
            isLogIn(email)
            var option = document.querySelector("#option-div");
            option.innerHTML = "";
            var viewCart = document.createElement("label");
            viewCart.innerText = "View cart";
            ...
            var signOut = document.createElement("label");
            ...
            option.appendChild(signOut);

            mainDiv.removeChild(container);
            CreateCardContainerComponent(ProductList());
...
function cardComponent(data) {
    ...
    cardContainer.setAttribute("class", "container-fluid mt-5");
    cardContainer.setAttribute("id", "card-container");

    for (var productIndex in data) {
        var productData = data[productIndex];

        var card = document.createElement('div');
        card.setAttribute("style", "width: 300px; height: 380px; border: 1px solid black; float:left; margin-left:45px;margin-bottom:20px");

        var heading = document.createElement('h5');
        heading.textContent = `${productData.title}`;
        heading.setAttribute('style', 'color: black; text-align: center; margin-top: 1%;');
        card.appendChild(heading);

        var productImg = document.createElement('img');
        productImg.setAttribute('src', productData.thumbnail);
        productImg.setAttribute("style", "height:170px;width:60%; margin-left:60px");
        card.appendChild(productImg);

        var Price = document.createElement('p');
        Price.setAttribute("style", "font-size:18px;text-align:center;");
        Price.innerHTML = `<br>Price : <b class='text-success'>${productData.price} Rs.</b>`
        card.appendChild(Price);

        var rating = document.createElement('p');
        rating.setAttribute("style", "font-size:18px;text-align:center;");
        rating.innerHTML = `Rating : <b class='text-warning'>${productData.rating}*</b>`
        card.appendChild(rating);

        var addToCart = document.createElement('button');
        addToCart.innerText = "ADD TO CART";
        addToCart.setAttribute("style", "width:85%; margin: auto; height: 30px; color: white");
        addToCart.setAttribute("class", "btn btn-secondary d-flex align-items-center justify-content-center");

        addToCart.addEventListener("click", (function (product) {
            return function () {
...
function saveProductInToCart(product) {
    var email = isLoggedIn() ? JSON.parse(sessionStorage.getItem("current-user")).email : null;

    if (email) {
        var cartList = JSON.parse(localStorage.getItem("cart-list")) || [];

        var existingProduct = cartList.find(item => item.productId === product.id);
        ...
        window.alert("Please sign in before adding products to the cart.");
    }
}
...
function addToCart() {
    ...
    var heading = document.createElement('h2');
    heading.setAttribute('style', 'text-align:center; margin-top:20px;');
    heading.innerText = 'SHOPPING CART';

    var headingTwo = document.createElement('h6');
    headingTwo.setAttribute('style', 'margin-left:50px; margin-top:10px');
    headDiv.appendChild(heading);
    headDiv.appendChild(headingTwo);
    cart.appendChild(headDiv);

    var productListInCart = JSON.parse(localStorage.getItem("cart-list")) || [];
    if (productListInCart.length === 0) {
        // Display message when the cart is empty
        var emptyCartMessage = document.createElement('p');
        emptyCartMessage.innerText = 'Oops! Cart is empty.';
        emptyCartMessage.setAttribute('style', 'text-align: center; font-size: 18px; margin-top: 20px;');
        cart.appendChild(emptyCartMessage);
    }
    else {
        // Display number count of items in the heading
        headingTwo.innerText = `Items In Cart: ${productListInCart.length}`;

        for (var productData of productListInCart) {
            // Check if productData has the expected properties
            if (productData.productthumbnail && productData.productPrice && productData.productTitle) {
                var productDiv = document.createElement('div');
                productDiv.setAttribute('style', 'border: 2px solid black; height: 200px; width: 500px; padding: 10px; margin-bottom: 10px; display: flex; align-items: center; margin-left:50px');

                var productImg = document.createElement('img');
                productImg.setAttribute('style', 'height:180px; width:200px; margin-right: 20px;');
                productImg.setAttribute('src', productData.productthumbnail);
                productDiv.appendChild(productImg);

                var textDiv = document.createElement('div');
                textDiv.setAttribute('style', 'flex-grow: 1;');

                var productName = document.createElement('h4');
                productName.setAttribute('style', 'margin-top: 0; margin-bottom: 5px;');
                productName.textContent = `${productData.productTitle}`;
                textDiv.appendChild(productName);

                var productPrice = document.createElement('h4');
                productPrice.setAttribute('style', 'margin-top: 0; margin-bottom: 5px;');
                productPrice.textContent = `Rs.${productData.productPrice}`;
                textDiv.appendChild(productPrice);

                var quantityLabel = document.createElement('p');
                quantityLabel.textContent = 'Quantity:';
                quantityLabel.setAttribute('style', 'margin-right: 10px; font-size: 16px;');
                textDiv.appendChild(quantityLabel);

                var quantityDisplay = document.createElement('span');
                quantityDisplay.textContent = '1'; // Default quantity
                quantityDisplay.setAttribute('style', 'margin-right: 10px; font-size: 16px;');
                textDiv.appendChild(quantityDisplay);

                var productCount = document.createElement('span');
                productCount.textContent = ` (${getProductCount(productData.productTitle)})`;
                productCount.setAttribute('style', 'font-size: 16px;');
                textDiv.appendChild(productCount);

                var incrementButton = document.createElement('button');
                incrementButton.innerText = '+';
                incrementButton.setAttribute('style', 'margin-right: 5px');
                incrementButton.addEventListener('click', function (display, count, title, productDiv) {
                    return function () {
                        // Handle incrementing quantity
                        var currentQuantity = parseInt(display.textContent);
                        display.textContent = currentQuantity + 1;
                        count.textContent = ` (${getProductCount(title)})`;
                    };
                }(quantityDisplay, productCount, productData.productTitle, productDiv));
                textDiv.appendChild(incrementButton);

                var decrementButton = document.createElement('button');
                decrementButton.innerText = '-';
                decrementButton.addEventListener('click', function (display, count, title, productDiv) {
                    return function () {
                        ...
                        if (currentQuantity > 1) {
                            ...
                            removeProductFromCart(productDiv, title);
                        }
                    };
                }(quantityDisplay, productCount, productData.productTitle, productDiv));
                textDiv.appendChild(decrementButton);
        ...
        generateBillButton.innerText = 'Generate Bill';
        generateBillButton.setAttribute('style', 'margin-left: 50px; margin-top: 20px; padding: 10px 20px; font-size: 16px;');
        generateBillButton.addEventListener('click', generateBill);
        cart.appendChild(generateBillButton);
    }

    mainDiv.appendChild(cart);

    function getProductCount(title) {
        // Helper function to get the count of a specific product in the cart
        var count = 0;
        for (var product of productListInCart) {
            if (product.productTitle === title) {
                count++;
            }
        }
        return count;
    ...
    function removeProductFromCart(productDiv, title) {
        productListInCart = productListInCart.filter(product => product.productTitle !== title);
        localStorage.setItem("cart-list", JSON.stringify(productListInCart));
        // Check if productDiv is still a child of the cart before attempting to remove it
        if (cart.contains(productDiv)) {
...
//     // Select the main div where the bill will be displayed
//     var mainDiv = document.querySelector("#main");
    
//     // Clear any previous bill details
//     mainDiv.innerHTML = "";

//     // Create a new div for the bill
...
//     // Append the heading to the headDiv and the headDiv to the bill div
//     headDiv.appendChild(heading);
//     bill.appendChild(headDiv);

//     // Fetch the product list from the local storage
//     var productListInCart = JSON.parse(localStorage.getItem("cart-list")) || [];
...
//     for (var productData of productListInCart) {
//         var productName = productData.productTitle;
//         var productPrice = parseFloat(productData.productPrice);
//         var quantity = getProductCount(productName);
...
//         priceCell.textContent = `Rs.${(productPrice * quantity).toFixed(2)}`;
...
function isLoggedIn() {
    // Assuming you want to check if the user is logged in based on some condition
